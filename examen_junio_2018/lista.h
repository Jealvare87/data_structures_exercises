...
public:
	...
	*/
	...
	void insertarPorPosicion(const T &elem, int pos){
		Lista<T>::Iterator it = this->begin();
		Lista<T>::Iterator antes;
		int i  = 0;
		...
		this->insertaElem(elem, antes, it);
		
 	}
	//
	void enlazaRaro(){
		Lista<T> aux = Lista<T>(this);
		Lista<T> aux1 = Lista<T>();
		while(!aux.esVacia()){
			aux1.pon_final(aux.principio());
			aux.quita_ppio();
			if(!aux.esVacia()){
				aux1.pon_final(aux.final());
				aux.quita_final();
			}
		}
	...
	void repartir() {
		...
		Nodo* siguiente = this->_prim->_sig;
		Nodo* ant_siguiente = nullptr;
		Nodo* ant_desp = this->_prim;
		while (siguiente != NULL) {
			if (siguiente->_elem < primero->_elem) {
				if (ant_siguiente != nullptr) { // Al anterior que vas a borrar le marcas el siguiente
					ant_siguiente->_sig = siguiente;
				}
				ant_siguiente = siguiente;
				ant_siguiente->_sig = primero;
				siguiente = siguiente->_sig;
				ant_desp->_sig = siguiente;
				//primero->_sig = siguiente;
			}
			else {
				ant_desp = siguiente;
				siguiente = siguiente->_sig;
	...
	void intercalar() {
		Nodo* primeros = this->_prim;
		Nodo* despues = this->_ult;
		while (primeros != NULL) {
			this->_ult = despues->_ant;
			despues->_ant = primeros;
			despues->_sig = primeros->_sig;
			if (primeros->_sig != NULL)primeros->_sig->_ant = despues;
			primero->_sig = despues;
			primero = despues->_sig;
			despues = this->_ult;
		}
	}
